...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('glitch')
    .setDescription('Glitch animation')
  ...
  async execute(interaction) {
    ...
    getImage.getImageAndCheckSize(img, interaction, async ({ error, url }) => {
      ...
      const caFrame = (padding, padding2, showNormal = true, color1 = 'rgba(255, 0, 251, 0.5)', color2 = 'rgba(0, 255, 30, 0.5)') => {
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        ctx.fillStyle = color1;
        ctx.drawImage(userImage, coords.x + padding, coords.y + padding, imgSize.width, imgSize.height);
        ...
        if (showNormal) ctx.drawImage(userImage, coords.x + padding2, coords.y - padding2, imgSize.width, imgSize.height);
