module.exports = {
  sendHTML: function(username, userAvatar, userId, repData, allPoints, pointsOnServer, message) {
    ...
    for(const item of repData) {
      tableData += `<tr><td class="${item.value == 1 ? 'g' : 'r'}">${item.value}</td><td>${item.reason}</td><td>${item.sender.username}</td><td>${item.sender.user_id}</td><td>${item.createdAt.toLocaleString(date_locale)}</td></tr>`;
    }
