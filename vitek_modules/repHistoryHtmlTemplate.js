module.exports = {
  generateRepHistoryHTML: function(username, userAvatar, userId, repData, allPoints, pointsOnServer, interaction) {
    ...
    for (const item of repData) {
      tableData += `<tr><td class="${item.value == 1 ? 'g' : 'r'}">${item.value}</td><td>${item.reason.replace(/([&<>"'])/g, match => htmlEntities[match])}</td><td>${item.sender.username}</td><td>${item.sender.user_id}</td><td>${item.createdAt.toLocaleString(dateLocale)}</td></tr>`;
    }
